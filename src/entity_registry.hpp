...
class entity_registry {
  public:
    ...
    std::vector<Component *> getComponents(uint32_t entityID) { return entities[entityID]; }
    ...
    template <class T> const std::vector<Component> &getComponents() { return components[typeid(T).name()]; }
