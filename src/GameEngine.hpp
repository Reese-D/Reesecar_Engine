...
// Optional, removes the need for alignas on UBO's, but doesn't work for nested
// structures. #define GLM_FORCE_DEFAULT_ALIGNED_GENTYPES
#include <chrono>
#include <glm/glm.hpp>
...
class GameEngine {
  public:
    GameEngine(std::shared_ptr<entity_registry> registry) : registry_(registry) {
        updateMeshes();
        updateInstances();
    }
    ...
    void run() {
        ...
        // note: image format is just an enum, don't need to recreate this if
        // swapchain dies/changes
  ...
  private:
    std::shared_ptr<entity_registry> registry_;
    ...
    void createTextureSampler() {
        ...
        // If you change this to false, make sure to remove the requirement for it
        // in device.cpp
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        ubo.projection[1][1] *= -1; // GLM was originally for openGL where the Y
                                    // coordinate was inversed for clip
    ...
    void createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer &buffer, VkDeviceMemory &bufferMemory) {
        ...
        // TODO: don't callf or every individual buffer. see comments at the end of
        // this chapter: https://vulkan-tutorial.com/Vertex_buffers/Staging_buffer
        // consider using
    ...
    template <typename T> void createBufferWithStaging(VkBuffer *buffer, VkDeviceMemory *memory, VkBufferUsageFlags flags_for_dst_transfer, std::vector<T> &bufData) {
        ...
        // now create another buffer, this time for the GPU only (not cpu
        // accessible)
        createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | flags_for_dst_transfer, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, *buffer, *memory);

        // copy the data from staging to supplied and cleanup the staging buffer
        // (caller must clean up buffer later)
    ...
    void copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
        // TODO: Look into using another command pool for short lived command
        // buffers like this
        ...
        // NOTE: could use a fence here to wait if multiple transfers are done
        // simultaneously. We're only doing 1 right now so we'll just execute
        // immediately
    }
    ...
    // Note only checks for the memory TYPE not the HEAP it comes from which can
    // make a big difference. For example, one heap might be swap space and
    // another from RAM.
    ...
    void createSyncObjects() {
        ...
        fenceInfo.flags = VK_FENCE_CREATE_SIGNALED_BIT; // first call to vkWaitForFences will
                                                        // immediately return
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex, VkRenderPass renderPass, std::vector<VkFramebuffer> swapChainFramebuffers) {
        VkCommandBufferBeginInfo beginInfo{};
        beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
        ...
        vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          graphicsPipeline_); // TODO get graphics pipeline
