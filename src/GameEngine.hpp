...
public:
  ...
  GameEngine(std::shared_ptr<entity_registry> registry)
    :registry_(registry)
  {
    updateMeshes();
    updateInstances();
...
private:
  ...
  void createTextureImage() {
    ...
    createImage((uint32_t)texWidth, (uint32_t)texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage_, textureImageMemory_);
